...
exports.postBoardData = async (req, res, next) => {
  const { bName, bDescription, bTotalNumber, bType, bStartDate, bClosingDate } =
    req.body;
  const userId = req.user.id;

  try {
    const exBoard = await Board.findOne({
      where: { bName, bDescription, bStartDate },
    });
    if (exBoard) {
...
exports.acceptBoardEnter = async (req, res, next) => {
  ...
  try {
    const request = await BoardRequest.findOne({
