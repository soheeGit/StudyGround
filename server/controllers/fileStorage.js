const { User, Board, File, FileStorage } = require('../models');
...
exports.updateFiles = async (req, res, next) => {
  ...
  try {
    ...
    const fileStorage = await FileStorage.findOne({
      where: { id: fileStorageId, userId: userId },
    });
    if (!fileStorage) {
    ...
    if (req.files && req.files.length > 0) {
      await File.destroy({
        where: { fileableType: 'FileStorage', fileableId: fileStorage.id },
      });
      uploadedFiles = req.files.map((file) => ({
        fileName: file.filename,
        fileUrl: `${process.env.CLIENT_URL}/files/${file.filename}`, // 파일 URL 생성
...
exports.getFileStorage = async (req, res, next) => {
  ...
  try {
    ...
    if (fileStorages.length === 0) {
      return res
        .status(200)
        .json({ message: '해당 스터디의 파일 저장소가 없습니다.' });
    }
...
exports.currentFile = async (req, res, next) => {
  const userId = req.user.id;
  const boardId = req.params.id;
  try {
    const user = await User.findOne({ where: { id: userId } });
